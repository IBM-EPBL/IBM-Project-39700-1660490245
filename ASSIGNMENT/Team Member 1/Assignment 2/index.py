...
def login():
    ...
    if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
        ...
        stmt = ibm_db.exec_immediate(conn,'SELECT * FROM KAVITHA_DB WHERE email = % s AND password = % s', (email, password, ))
...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form and 'roll_no' in request.form :
        ...
        stmt = ibm_db.exec_immediate(conn,'SELECT * FROM KAVITHA_DB')
        if email==cpassword:
            ...
            sql=ibm_db.exec_immediate(conn,'INSERT INTO KAVITHA_DB (roll_no, username, email, password, cpassword) VALUES (%s, % s, % s, % s)')
